...
namespace Twobob.Mm2
{
    ...
    public class SplineClamp : Generator, IInlet<SplineSys>, IOutlet<SplineSys>
    {
        ...
        static void EnlistInMenu() =>
            MapMagic.Nodes.GUI.CreateRightClick.generatorTypes.Add(typeof(SplineClamp));
#endif
        ...
        public override void Generate(TileData data, StopToken stop)
        {
            ...
            if (spline == null || !enabled)
                return;

            if (stop != null && stop.stop)
                return;
            if (clamp == Clamp.Full)
                spline.Clamp((Vector3)data.area.full.worldPos, (Vector3)data.area.full.worldSize);
            if (clamp == Clamp.Active)
                spline.Clamp(
                    (Vector3)data.area.active.worldPos,
                    (Vector3)data.area.active.worldSize
                );
