    ...
    public class MultiFloor
    {
        ...
        public class MultiFloor200 : Generator, IMultiInlet, IMultiOutlet // IOutlet<SplineSys>
        {
            ...
            public IEnumerable<IInlet<object>> Inlets()
            {
                ...
                yield return spline1In;
                yield return spline2In;
                yield return spline3In;
                yield return spline4In;
                yield return spline5In;
                yield return spline6In;
                yield return spline7In;
                yield return spline8In;
            }
            public IEnumerable<IOutlet<object>> Outlets()
            {
                yield return spline1Out;
                yield return spline2Out;
                yield return spline3Out;
                yield return spline4Out;
                yield return spline5Out;
                yield return spline6Out;
                yield return spline7Out;
                yield return spline8Out;
            }

#if UNITY_EDITOR
            [UnityEditor.InitializeOnLoadMethod]
            static void EnlistInMenu() =>
                MapMagic.Nodes.GUI.CreateRightClick.generatorTypes.Add(typeof(MultiFloor200));
#endif
            ...
            public void ProcessToFloor(
                Inlet<SplineSys> splineIn,
                Outlet<SplineSys> splineOut,
                TileData data,
                StopToken stop
            )
            {
                ...
                {
                    data.StoreProduct(splineOut, src);
                    return;
                }
            ...
            public static void FloorSplines(SplineSys dst, MatrixWorld heights)
            {
                foreach (Line line in dst.lines)
                {
                    for (int s = 0; s < line.segments.Length; s++)
                        line.segments[s].start.pos.y = FloorPoint(
                            line.segments[s].start.pos,
                            heights
                        );

                    line.segments[line.segments.Length - 1].end.pos.y = FloorPoint(
                        line.segments[line.segments.Length - 1].end.pos,
                        heights
                    );
                }
            }
            ...
            {
                if (pos.x <= heights.worldPos.x)
                    pos.x = heights.worldPos.x + 0.001f;
                if (pos.x >= heights.worldPos.x + heights.worldSize.x)
                    pos.x = heights.worldPos.x + heights.worldSize.x - 0.001f;

                if (pos.z <= heights.worldPos.z)
                    pos.z = heights.worldPos.z + 0.001f;
                if (pos.z >= heights.worldPos.z + heights.worldSize.z)
                    pos.z = heights.worldPos.z + heights.worldSize.z - 0.001f;
