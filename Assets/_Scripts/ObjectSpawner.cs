...
public class ObjectSpawner : MonoBehaviour
{
    ...
    private class SpawnableObject
    {
        ...
        {
            get { return m_SpawnScale; }
        }
        public int SpawnAmount
        {
            get { return m_SpawnAmount; }
        }
    }

    // Internal variables
    [SerializeField]
    private SpawnableObject[] m_SpawnObjects;
    [SerializeField]
        ...
        for (int i = 0; i < m_SpawnObjects.Length; ++i)
        {
            for (int j = 0; j < m_SpawnObjects[i].SpawnAmount; ++j)
            {
                ...
                Vector3 offset = new Vector3(
                    (int)UnityEngine.Random.Range(-RandomOffset.x, RandomOffset.x),
                ...
                var obj =
                    GameObject.Instantiate(
                        m_SpawnObjects[i].Object,
                        transform.position + m_SpawnObjects[i].Position + offset,
                        m_SpawnObjects[i].Rotation
                    ) as GameObject;
