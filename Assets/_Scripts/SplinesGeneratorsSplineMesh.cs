...
namespace Twobob.Mm2
{
    ...
    public class SplineMeshControl : Generator, IInlet<SplineSys>, IOutlet<SplineSys>
    {
        ...
        static void EnlistInMenu() =>
            MapMagic.Nodes.GUI.CreateRightClick.generatorTypes.Add(typeof(SplineMeshControl));
#endif

        [Val("Input", "Inlet")]
        public readonly Inlet<SplineSys> splineIn = new Inlet<SplineSys>();
        ...
        [Val("Type")]
        public objectRendered chosenType = objectRendered.wall;

        [Val("TryToFloor")]
        public bool TryToFloor = false;

        public IEnumerable<IInlet<object>> Inlets()
        {
            yield return splineIn;
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage(
            "Type Safety",
            "UNT0010:MonoBehavior instance creation is not recommended",
        ...
        public override void Generate(TileData data, StopToken stop)
        {
            ...
            foreach (var item in dst.lines)
            {
                ...
                for (int segmentindex = 0; segmentindex < item.segments.Length; segmentindex++)
                {
                    ...
                    nodesInThisSegmentGroup = AddNodeToListReturnIncrementedTotalCount(
                        nodesInThisSegmentGroup,
                        segmentToProcess,
                        typed.nodes
                    );
        ...
        private static int AddNodeReturnIncrementedTotalCount(
            TileData data,
            int nodesInThisSegmentGroup,
            Segment segmentToProcessPos,
            SplineMesh.Spline mysm
        )
        {
            ...
            mysm.AddNode(
                new SplineMesh.SplineNode(
                    segmentToProcessPos.start.pos,
                    segmentToProcessPos.start.pos
                )
            );
            mysm.AddNode(
                new SplineMesh.SplineNode(segmentToProcessPos.end.pos, segmentToProcessPos.end.pos)
            );
        ...
        private static int AddNodeToListReturnIncrementedTotalCount(
            int nodesInThisSegmentGroup,
            Segment segmentToProcessPos,
            List<SplineMesh.SplineNode> mysm
        )
        {
            mysm.Add(
                new SplineMesh.SplineNode(
