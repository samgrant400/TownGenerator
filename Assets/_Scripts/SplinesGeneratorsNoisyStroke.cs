...
namespace MapMagic.Nodes.SplinesGenerators
{
    ...
    public class NoisyStroke : Generator, IInlet<SplineSys>, IOutlet<MatrixWorld>
    {
        ...
        static void EnlistInMenu() =>
            MapMagic.Nodes.GUI.CreateRightClick.generatorTypes.Add(typeof(NoisyStroke));
#endif
        ...
        public override void Generate(TileData data, StopToken stop)
        {
            ...
            if (stop != null && stop.stop)
                return;
            MatrixWorld strokeMatrix = new MatrixWorld(
                data.area.full.rect,
            ...
            MatrixWorld noiseWorld = new MatrixWorld(
                spreadMatrix.rect,
                spreadMatrix.worldPos,
                spreadMatrix.worldSize
            );
            noiseWorld.Fill(0);
...

            {
                for (int x = min.x; x < max.x; x++)
                    for (int z = min.z; z < max.z; z++)
                    {
                        ...
                        int pos =
                            (z - noiseWorld.rect.offset.z) * noiseWorld.rect.size.x
                            + x
                            - noiseWorld.rect.offset.x; //coord.GetPos
                        ...
                        if (noiseFallof)
                        {
                            float maxNoise = 1 - spreadMatrix.arr[pos];
                            if (spreadMatrix.arr[pos] < 0.5f)
                                maxNoise = spreadMatrix.arr[pos];
                            falloff +=
                                (data.random.Fractal(x, z, noiseSize) * 2 - 1)
                                * maxNoise
                                * noiseAmount;
                        }

                        noiseWorld.arr[pos] = spreadMatrix.arr[pos] * falloff;
                    }
            }
        ...
        public static MatrixWorld Spread(MatrixWorld matrix, float range)
        {
            ...
            else //spreading the usual way
            {
                ...
                MatrixOps.SpreadLinear(
                    matrix,
                    spreadMatrix,
                    subtract: 1f / pixelRange,
                    diagonals: true,
                    quarters: true
                );
            }
