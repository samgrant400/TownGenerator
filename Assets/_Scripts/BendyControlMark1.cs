...
namespace MapMagic.Nodes.SplinesGenerators
{
    ...
    {
        [Val("Input", "Inlet")]
        public readonly Inlet<SplineSys> input = new Inlet<SplineSys>();
        ...
        static void EnlistInMenu() =>
            MapMagic.Nodes.GUI.CreateRightClick.generatorTypes.Add(typeof(BendyV1));
#endif
            ...
            for (int i = 0; i < bend.lines.Length; i++)
            {
                for (int j = 0; j < bend.lines[i].segments.Length; j++)
                {
                    ...
                    if (j > 0)
                    {
                        ...
                        if (useNoise)
                        {
                            Vector3 place = ReturnWigglyVector3UsingPerlinNoise(
                                bendiness,
                                startpos.V3(),
                                false
                            );
                            ...
                            start.dir = //place;
                                (FlipALocationCoin(startpos)) ? start.dir + place : start.dir - place;
                        }
                        else
                        {
                    ...
                    if (j < bend.lines[i].segments.Length)
                    {
                        ...
                        if (useNoise)
                        {
                            Vector3 place = ReturnWigglyVector3UsingPerlinNoise(
                                ...
                                (FlipALocationCoin(endpos)) ? end.dir + place : end.dir - place;
                        }
                        else
                        {
                            Vector3 place = ReturnWigglyVector3(bendiness);
        ...
        private Vector3 ReturnWigglyVector3UsingPerlinNoise(
            float factor,
            Vector3 location,
            bool addOffset
        )
        ...
        {
            return new Vector3(
                RandomGen.Next(10, -10) * 0.1f * factor,
                0,
                RandomGen.Next(10, -10) * factor
            );
        }

        private Vector3 ReturnWigglyVector3UsingFractalNoise(
            float factor,
            Vector2 location,
            Noise randomnoise
        )
        ...
        private float ReturnFractalNoiseValueAtSpot(
            float factor,
            Vector2 location,
            Noise randomnoise
        )
