...
namespace Twobob.Mm2
{
    [System.Serializable]
    [GeneratorMenu(
        menu = "Objects/Initial",
        name = "GlobalRandom",
        iconName = "GeneratorIcons/Random",
        disengageable = true,
    ...
    public class GlobalRandomObjectsMark1 : Generator, IInlet<MatrixWorld>, IOutlet<TransitionsList>
    {
        ...
        static void EnlistInMenu() =>
            MapMagic.Nodes.GUI.CreateRightClick.generatorTypes.Add(
                typeof(GlobalRandomObjectsMark1)
            );
#endif
        ...
        public override void Generate(TileData data, StopToken stop)
        {
            ...
            PosTab posTab = new PosTab(
                ...
                (Vector3)data.area.full.worldSize * factor,
                16
            );
            RandomScatter(
                (int)count,
        ...
        public static void RandomScatter(
            int count,
            float uniformity,
            Vector3 offset,
            Vector3 size,
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                for (int c = 0; c < candidatesNum; c++)
                {
                    float candidateX =
                        ...
                        ); //TODO: do not use pos since it changes between preview/full
                    float candidateZ =
                        (offset.z + 1)
                        + (
                            rnd.Random(
                    ...
                    float dist =
                        (closest.pos.x - candidateX) * (closest.pos.x - candidateX)
                        + (closest.pos.z - candidateZ) * (closest.pos.z - candidateZ);

                    //distance to the edge
                    float bd = (candidateX - offset.x) * 2;
                    if (bd * bd < dist)
                        dist = bd * bd;
                    bd = (candidateZ - offset.z) * 2;
                    if (bd * bd < dist)
                        dist = bd * bd;
                    bd = (offset.x + size.x - candidateX) * 2;
                    if (bd * bd < dist)
                        dist = bd * bd;
                    bd = (offset.z + size.z - candidateZ) * 2;
                    if (bd * bd < dist)
                        dist = bd * bd;
