...
namespace MapMagic.Nodes.SplinesGenerators
{
    ...
    public class WigglerMini : Generator, IInlet<SplineSys>, IOutlet<SplineSys>
    {
        ...
        static void EnlistInMenu() =>
            MapMagic.Nodes.GUI.CreateRightClick.generatorTypes.Add(typeof(WigglerMini));
#endif
        ...
        public override void Generate(TileData data, StopToken stop)
        {
            ...
            foreach (var item in dst.lines)
            {
                ...
                for (int i = 1; i < item.NodesCount - 1; i++)
                {
                    ...
                    {
                        nv = new Vector3(
                            cur.x + ReturnWiggly(wiggliness),
                            cur.y,
                            cur.z + ReturnWiggly(wiggliness)
                        );
        ...
        {
            return new Vector3(
                RandomGen.Next(10, -10) * 0.1f * factor * scalar,
                0,
                RandomGen.Next(10, -10) * factor * scalar
            );
        }
