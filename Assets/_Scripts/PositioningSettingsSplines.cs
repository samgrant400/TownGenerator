...
namespace Twobob.Mm2
{
    ...
    [System.Serializable]
    public class PositioningSettingsSpline
	...
	/// Not in objects but in Nodes since it's used by Brush core
    {
        ...
        /// Floors object, and erases (yep) trs roation/scale values according to layer settings
        {
            ...
            if (relativeHeight && data.heights != null) //if checbox enabled and heights exist (at least one height generator is in the graph)
                terrainHeight = data.heights.GetWorldInterpolatedValue(
                    trs.pos.x,
                    trs.pos.z,
                    roundToShort: true
                );
            ...
            if (!useScale)
                trs.scale = new Vector3(1, 1, 1);
            else if (scaleYonly)
                trs.scale = new Vector3(1, trs.scale.y, 1);
            ...
            else if (takeTerrainNormal)
            {
                Vector3 terrainNormal = GetTerrainNormal(
                    ...
                    data.heights,
                    data.globals.height,
                    data.area.PixelSize.x
                );
                Vector3 terrainTangent = Vector3.Cross(
                    trs.rotation * new Vector3(0, 0, 1),
                    terrainNormal
                );
                trs.rotation = Quaternion.LookRotation(terrainTangent, terrainNormal);
            }
            else if (rotateYonly)
                trs.rotation = Quaternion.Euler(0, trs.Yaw, 0);
        }

        public static Vector3 GetTerrainNormal(
            float fx,
            float fz,
            MatrixWorld heightmap,
            float heightFactor,
            float pixelSize
        )
        {
            ...
            if (coord.x >= heightmap.rect.offset.x + 1)
                prevXHeight = heightmap.arr[pos - 1];

            float nextXHeight = curHeight;
            if (coord.x <= heightmap.rect.offset.x + heightmap.rect.size.x - 1)
                nextXHeight = heightmap.arr[pos + 1];

            float prevZHeight = curHeight;
            if (coord.z >= heightmap.rect.offset.z + 1)
                prevZHeight = heightmap.arr[pos - heightmap.rect.size.x];

            float nextZHeight = curHeight;
            if (coord.z <= heightmap.rect.offset.z + heightmap.rect.size.z - 1)
                nextZHeight = heightmap.arr[pos + heightmap.rect.size.z];

            return new Vector3(
                (prevXHeight - nextXHeight) * heightFactor,
                pixelSize * 2,
                (prevZHeight - nextZHeight) * heightFactor
            ).normalized;
        }

        public static bool SkipOnBiome(
            ref Transition trs,
        ...
        {
            float biomeFactor =
                biomeMask != null ? biomeMask.GetWorldInterpolatedValue(trs.pos.x, trs.pos.z) : 1;
