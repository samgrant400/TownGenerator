...
namespace Twobob.Mm2
{
    ...
    public class HoleOutMark1 : OutputGenerator, IInlet<MatrixWorld>
    {
        ...
        static void EnlistInMenu() =>
            MapMagic.Nodes.GUI.CreateRightClick.generatorTypes.Add(typeof(HoleOutMark1));
#endif
        ...
        public static void Finalize(TileData data, StopToken stop)
        {
            ...
            int matrixRes =
                (data.heights.rect.size.x - margins * 2 - 1) * upscale + margins * 2 * upscale + 1;
            ...
            foreach (
                (HoleOutMark1 output, MatrixWorld product, MatrixWorld biomeMask) in data.Outputs<
                    HoleOutMark1,
                    MatrixWorld,
                    MatrixWorld
                >(typeof(HoleOutMark1), inSubs: true)
            )
            {
                ...
                Coord heightsSize = new Coord(
                    heights2Dfull.GetLength(1),
                    heights2Dfull.GetLength(0)
                ); //x and z swapped
                ...
                Coord min = intersection.Min;
                Coord max = intersection.Max;

                for (int x = min.x; x < max.x - 1; x++)
                    for (int z = min.z; z < max.z - 1; z++)
                    {
                        int matrixPos =
                            (z - matrixbool.rect.offset.z) * matrixbool.rect.size.x
                            + x
                            - matrixbool.rect.offset.x;
        ...
        public class ApplySetData : IApplyData, IApplyHeightData
        {
            ...
            {
                heights2D = terrain.terrainData.GetHeights(
                    rect.offset.x,
                    rect.offset.z,
                    rect.size.x,
                    rect.size.z
            ...
            public void Apply(Terrain terrain)
            {
                if (terrain == null || terrain.Equals(null) || terrain.terrainData == null)
                    return; //chunk removed during apply
            ...
            {
                get { return heights2D.GetLength(0); }
            }
        }
    }
}
