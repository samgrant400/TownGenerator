...
namespace Town
{
    ...
    public class CastleArea : TownArea
    {
        ...
        protected override void GetBuildingSpecs(
            out float gridChaos,
            out float sizeChaos,
            out Func<Polygon, float> emptyProbabilityFunc,
            out float minArea
        )
        {
            minArea = 200 + 200 * random.value * random.value;
            gridChaos = 0;
            sizeChaos = 0;
    ...
    public class PoorArea : TownArea
    {
        ...
        {
            minArea = 10 + 30 * random.value * random.value;
            gridChaos = 0.2f + random.value * 0.02f;
            sizeChaos = 1f;
    ...
    public class OutsideWallArea : TownArea
    {
        ...
        )
        {
            ...
            sizeChaos = 1f;
            var closestWallPoint = Patch.Town.CityWall.Circumference
                .OrderBy(p => (p - Patch.Center).Length)
                .First();
            ...
            emptyProbabilityFunc = p =>
            {
                ...
                var normalizedDistanceSq = (float)Math.Pow(
                    distanceToWall / (1.5f * centerDistance),
                    3
                );
    ...
    public class RichArea : TownArea
    {
        ...
        {
            minArea = 60 + 160 * random.value * random.value;
            gridChaos = 0.05f + random.value * 0.02f;
            sizeChaos = 0.2f;
    ...
    public class TownArea : Area
    {
        ...
        private Polygon GetCityBlock()
        {
            ...
            foreach (var edge in Patch.Edges)
            {
                ...
                else
                {
                    ...
                    var onStreet =
                        streetVertices.Contains(edge.A) && streetVertices.Contains(edge.B);
                    if (!onStreet)
                    {
        ...
        private IEnumerable<Polygon> CreateAlleys(
            Polygon block,
            float minArea,
            float gridChaos,
            float sizeChaos,
        ...
        )
        {
            ...
            var angleSpread = (float)(
                Math.PI / 6 * gridChaos * (block.Area() < minArea * 4 ? 0 : 1)
            );
            var angle = (random.value - 0.5f) * angleSpread;
            ...
            {
                if (
                    half.Area() < minArea * Math.Pow(2, 4 * sizeChaos * (random.value - 0.5f))
                    || levels > 5
                )
                {
                    if (!random.Boolean(emptyProbabilityFunc(half)))
                    {
                        buildings.Add(half);
                    }
                }
                else
                {
                    buildings.AddRange(
                        CreateAlleys(
                            ...
                            gridChaos,
                            sizeChaos,
                            emptyProbabilityFunc,
                            half.Area() > minArea / (random.value * random.value),
                            levels + 1
                        )
                    );
                }
        ...
        private IEnumerable<Polygon> GetBuildings()
        {
            ...
            GetBuildingSpecs(
                out var gridChaos,
                out var sizeChaos,
                out var emptyProbabilityFunc,
                out var minArea
            );

            foreach (
                var building in CreateAlleys(
                    block,
                    minArea,
                    gridChaos,
                    sizeChaos,
        ...
        protected virtual void GetBuildingSpecs(
            out float gridChaos,
            out float sizeChaos,
            out Func<Polygon, float> emptyProbabilityFunc,
            out float minArea
        ...
        {
            minArea = 10 + 80 * random.value * random.value;
            gridChaos = 0.05f + random.value * 0.02f;
            sizeChaos = 0.06f;
