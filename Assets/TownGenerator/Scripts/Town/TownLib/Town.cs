...
namespace Town
{
    public class Town
    {
        ...
        // have mercy
        public List<
            Dictionary<Den.Tools.Coord, List<UnityEngine.Vector3>>
        > SplinesToRender { get; set; }
        ...
        public Town(TownOptions options)
        {
            ...
            while (!ok)
            {
                ...
                Center = new Vector2(
                    Width / 2f * (1f + 0.1f * Random.value - 0.1f * Random.value),
                    Height / 2f * (1f + 0.1f * Random.value - 0.1f * Random.value)
                );
        ...
        private void BuildPatches()
        {
            ...
            var patchesInTown = Patches
                .OrderBy(p => (Center - p.Center).Length)
                .Take(NumPatches)
                .ToList();
            ...
            if (Options.Water)
            {
                var firstWaterPatch = patchesInTown
                    .Where(p => p.GetAllNeighbours().Any(n => !n.WithinCity))
                ...
                while (toCheck.Any())
                {
                    ...
                    var waterPatches = checking
                        .GetAllNeighbours()
                        .Except(patchesInTown)
                        .Where(
                            n =>
                                Math.Abs((Center - n.Center).AngleComparedTo(waterDirection))
                                < Math.PI / 4
                        )
                        .Where(n => !n.Water)
                        .ToList();
        ...
        private void OptimizePatches()
        {
            ...
            foreach (var patch in patchesToOptimize)
            {
                for (var i = 0; i < patch.Shape.Vertices.Count; i++)
                {
                    ...
                    {
                        var shapesWithPoint = Patches
                            .Where(p => p.Shape.Vertices.Contains(p1))
                            .Select(p => p.Shape);
        ...
        private void BuildWalls()
        {
            ...
            if (Options.Water)
            {
                ...
                var seaPoints = allowedTowerPositions
                    .Where(t => Patches.Any(p => p.Water && p.Shape.Vertices.Contains(t)))
                    .ToList();
                ...
                var minIndex = Math.Min(
                    allowedTowerPositions.IndexOf(lastSeaPoint),
                    allowedTowerPositions.IndexOf(firstSeaPoint)
                );
        ...
        private void BuildRoads()
        {
            ...
            foreach (var gate in Gates.ToList())
            {
                ...
                if (street != null)
                {
                    ...
                    if (CityWall.Gates.Contains(gate))
                    {
                        ...
                        var start = topology.Node2V.Values
                            .OrderBy(v => (v - direction).Length)
                            .First();
            ...
            {
                var insideVertices = road.Where(
                        rv => Patches.Where(p => p.Shape.ContainsPoint(rv)).All(p => p.WithinCity)
                    )
                    .Except(Gates)
                    .ToList();
        ...
        private List<List<Vector2>> TidyUpRoads(List<List<Vector2>> roads)
        {
            ...
            foreach (var tidyRoad in arteries)
            {
                ...
                for (var i = 0; i < tidyRoad.Count; i++)
                {
                    ...
                    var affectedPatches = Patches
                        .Where(p => p.Shape.Vertices.Contains(originalPoint))
                        .ToList();
        ...
        public static Polygon FindCircumference(IEnumerable<Patch> patches)
        {
            ...
            var edgeCounts = allEdges
                .Select(e => new { Edge = e, Count = allEdges.Count(e.Equals) })
                .ToList();
