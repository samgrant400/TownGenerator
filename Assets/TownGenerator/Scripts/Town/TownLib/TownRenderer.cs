...
namespace Town
{
    public class TownRenderer
    {
        ...
        public string DrawTown()
        {
            ...
            sb.Append(
                $"<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"1500\" height=\"1500\" viewBox=\"{bounds.ToSvgViewport()}\">"
            );

            sb.Append(@"<defs><style type=""text/css""><![CDATA[");
            sb.Append(".building { stroke-width: 0.1; stroke: #777; fill: #DDD; }");
            sb.Append(
                ".building-description { visibility: hidden; font-size: 4; font-family: Verdana; fill: #666; }"
            );
            ...
            sb.Append(
                ".overlay { fill: #FFFF00; fill-opacity: 0.2; stroke: black; stroke-width: 0.5; }"
            );
            ...
            {
                sb.Append(
                    "<text z-index=\"9\" id=\"building"
                        + id
                        + "\" class=\"building-description\" x=\""
                        + building.Shape.Vertices[0].x
                        + "\" y=\""
                        + building.Shape.Vertices[0].y
                        + "\">"
            ...
            sb.Append(
                $"<text x=\"{bounds.X + 20}\" y=\"{bounds.Y + 30}\">" + _options.Seed + "</text>"
            );
        ...
        private static void DrawWalls(TownGeometry geometry, StringBuilder sb)
        {
            ...
            foreach (var wall in geometry.Walls)
            {
                ...
                sb.Append(
                    $"<line x1=\"{start.x}\" y1=\"{start.y}\" x2=\"{end.x}\" y2=\"{end.y}\" class=\"wall\" />"
                );
            }
            ...
            {
                sb.Append(
                    $"<rect width=\"8\" height=\"8\" x=\"{tower.x - 4}\" y=\"{tower.y - 4}\" class=\"tower\" />"
                );
            }
            ...
            {
                sb.Append(
                    $"<rect width=\"8\" height=\"8\" x=\"{gate.x - 4}\" y=\"{gate.y - 4}\" class=\"gate\" />"
                );
            }
        }
    }
