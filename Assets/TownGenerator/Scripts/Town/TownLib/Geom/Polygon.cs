...
namespace Town.Geom
{
    public class Polygon : IEquatable<Polygon>
    {
        ...
        public bool IsConvex
        {
            get
            {
                foreach (var vertex in Vertices)
                {
                    ...
                    var crossProduct = GeometryHelpers.CrossProduct(
                        vertex.x - prev.x,
                        vertex.y - prev.y,
                        next.x - vertex.x,
                        next.y - vertex.y
        ...
        public int GetVertexIndex(Vector2 vertex)
        {
            ...
            if (idx < 0)
            {
                throw new InvalidOperationException(
                    "Vertex " + vertex + " is not part of the polygon!"
                );
            }
            return idx;
        }
        ...
        public IEnumerable<Polygon> Split(int i1, int i2)
        {
            ...
            var secondVertices = Vertices
                .GetRange(i2, Vertices.Count - i2)
                .Concat(Vertices.Take(i1 + 1));

            return new[] { new Polygon(firstVertices), new Polygon(secondVertices) };
        }
            ...
            do
            {
                ...
                for (var i = lastEdge; i < n - 2; i++)
                {
                    ...
                    for (var j = i + 2; j < maxJ; j++)
                    {
                        ...
                        var intersect = GeometryHelpers.IntersectLines(
                            x1,
                            y1,
                            dx1,
                            dy1,
        ...
        public IEnumerable<Polygon> Cut(Vector2 p1, Vector2 p2, float gap)
        {
            ...
            if (count == 2)
            {
                ...
                var half2 = new Polygon(
                    Vertices
                        .GetRange(edge2 + 1, Vertices.Count - edge2 - 1)
                        .Concat(Vertices.GetRange(0, edge1 + 1))
                );
        ...
        public IEnumerable<Polygon> Bisect(
            Vector2 vertex,
            float ratio = 0.5f,
            float angle = 0.0f,
            float gap = 0.0f
        ...
        public bool ContainsPoint(Vector2 point)
        {
            ...
            while (i < polygonLength)
            {
                ...
                //
                inside ^=
                    (endY > pointY ^ startY > pointY) /* ? pointY inside [startY;endY] segment ? */
                    &&
        ...
        {
            return Vertices
                .Select((v, i) => new { a = AngleAtVertex(i), i })
                .Where(t => Math.Abs(t.a) < Math.PI / 4)
                .Select(t => t.i);
        }
        ...
        public Polygon Simplify(float areaThreshold = 0.5f, float distanceThreshhold = 2f)
        {
            var toRemove = Vertices
                ...
                .Where(v => v.a < areaThreshold)
                .Select(v => v.i)
                .ToList();

            var simplified = Vertices.Where((v, i) => !toRemove.Contains(i)).Distinct().ToList();

            var tooClose = simplified
                .Where(
                    v1 =>
                        simplified
                            .Where(v2 => !v2.Equals(v1))
                            .Any(v2 => (v2 - v1).Length < distanceThreshhold)
                )
                .ToList();
            while (tooClose.Any())
            {
        ...
        public Polygon RectangleInside()
        {
            ...
            var newHouse = new Polygon(
                p1,
                p2,
                p2 - Vector2.Scale(normal, greatestDistance),
                p1 - Vector2.Scale(normal, greatestDistance)
            ).ZoomShrink(0.2f);
