...
namespace Town
{
    public class Wall
    {
        ...
        public Wall(
            List<Vector2> allowedTowerPositions,
            Town town,
            int minNumberOfGates,
            int maxNumberOfGates,
            List<Vector2> illegalGatePositions = null
        )
        {
            _town = town;
            ...
            PlaceGatesAndTowers(
                allowedTowerPositions,
                minNumberOfGates,
                maxNumberOfGates,
                illegalGatePositions
            );
        }
        ...
        private void PlaceGatesAndTowers(
            IEnumerable<Vector2> allowedTowerPositions,
            int minNumberOfGates,
            int maxNumberOfGates,
            List<Vector2> illegalGatePositions
        )
        {
            ...
            var attempts = 0;
            while (
                (gates.Count < minNumberOfGates || attempts < 4)
                && possibleGates.Any()
                && gates.Count < maxNumberOfGates
            )
            {
                attempts++;
                ...
                try
                {
                    ...
                    if (outerNeighbours.Count == 1)
                    {
                        ...
                        var wallPoint =
                            neighbour.Shape.GetNextVertex(newGate)
                            - neighbour.Shape.GetPreviousVertex(newGate);
                        var outPoint = new Vector2(wallPoint.y, -wallPoint.x);
                        var possibleSplitPoints = neighbour.Shape.Vertices
                            .Except(Circumference)
                            .ToList();
                        var farthest = possibleSplitPoints
                            .OrderByDescending(
                                p =>
                                {
                                    var dir = p - newGate;
                        ...
                        var newPatches = neighbour.Shape
                            .Split(newGate, farthest)
                            .Select(p => Patch.FromPolygon(_town, p))
                            .ToList();
                        if (newPatches.Any(p => p.Shape.Vertices.Count < 3))
                        {
                            farthest = neighbour.Shape.GetNextVertex(farthest);
                            newPatches = neighbour.Shape
                                .Split(newGate, farthest)
                                .Select(p => Patch.FromPolygon(_town, p))
                                .ToList();
                            if (newPatches.Any(p => p.Shape.Vertices.Count < 3))
