...
{
    public static partial class SplinesEditor
    {
        public static void DrawObjectPrefabs(
            ref GameObject[] prefabs,
            bool multiPrefab = true,
            bool treeIcon = false
        )
        {
            ...
            if (multiPrefab)
                using (Cell.LineStd)
                {
                    ...
                    LayersEditor.DrawLayers(
                        ref prefabs,
                        onDraw: n => DrawObjectPrefabLayer(prefabsCopy, n, iconName)
                    );
                }
            else
            {
                ...
                {
                    using (Cell.RowPx(24))
                        Draw.Icon(UI.current.textures.GetTexture(iconName));
                    Cell.EmptyRowPx(4);
                    using (Cell.Row)
                        Draw.Field(ref prefabs[0]);
                    Cell.EmptyRowPx(4);
                }
            }
        ...
        private static void DrawObjectPrefabLayer(GameObject[] prefabs, int n, string iconName)
        {
            ...
            using (Cell.LineStd)
            {
                using (Cell.RowPx(24))
                    Draw.Icon(UI.current.textures.GetTexture(iconName));
                using (Cell.Row)
                    prefabs[n] = Draw.ObjectField(prefabs[n]);
                Cell.EmptyRowPx(4);
            }
            Cell.EmptyLinePx(4);
        }

        public static void DrawPositioningSettings(
            PositioningSettingsSpline posSettings,
            bool billboardRotWaring = false,
            bool showRelativeHeight = true
        )
        {
            //  height
            ...
            using (new Draw.FoldoutGroup(ref posSettings.guiRotation, "Rotation"))
                ...
                {
                    using (Cell.LineStd)
                        Draw.ToggleLeft(ref posSettings.isRandomYaw, "Random Yaw?");
            ...
            using (new Draw.FoldoutGroup(ref posSettings.guiPositionSettings, "Mesh Options"))
                if (posSettings.guiPositionSettings)
                {
                    ...
                    using (Cell.LineStd)
                        Draw.ToggleLeft(ref posSettings.mergeSegments, "Segment Merge?");
                    using (Cell.LineStd)
                        Draw.ToggleLeft(ref posSettings.spacingFromScale, "Spacing from Scale");
                    ...
                    using (Cell.LineStd)
                        Draw.Field(ref posSettings.spacingRange, "Spacing Range");

                    using (Cell.LineStd)
                        Draw.Field(ref posSettings.offset, "Start Offset");

                    using (Cell.LineStd)
                        Draw.Field(ref posSettings.offsetRange, "Offset Range");
                    //using (Cell.LineStd)
                    //{
                    //	Cell.EmptyRowPx(18);
            ...
            using (new Draw.FoldoutGroup(ref posSettings.guiScale, "Scale"))
                if (posSettings.guiScale)
                {
                    ...
                    using (Cell.LineStd)
                        Draw.Field(ref posSettings.scale, "Mesh Scale");

                    using (Cell.LineStd)
                        Draw.Field(ref posSettings.scaleRange, "Scale Range");
                    //using (Cell.LineStd)
                    //{
                    //	Cell.EmptyRowPx(18);
                    //	using (Cell.Row) Draw.Label("Scale");
