    ...
    public class SplineExtrusionEditor : Editor
    {
        ...
        void OnSceneGUI()
        {
            ...
            foreach (ExtrusionSegment.Vertex v in se.shapeVertices)
            {
                ...
                Vector3 normal = se.transform.TransformPoint(
                    q * (v.point + v.normal) + startSample.location
                );
                ...
                if (v == selection)
                {
                    ...
                    Vector3 movedPoint = Handles.Slider2D(
                        0,
                        point,
                        startSample.tangent,
                        Vector3.right,
                        ...
                        new Vector2(snap, snap)
                    );
                    if (movedPoint != point)
                    {
                        // position has been moved
                        Vector2 newVertexPoint =
                        ...
                        if (mustCreateNewNode)
                        {
                            ...
                            mustCreateNewNode = false;
                            ExtrusionSegment.Vertex newVertex = new ExtrusionSegment.Vertex(
                                newVertexPoint,
                                v.normal,
                                v.uCoord
                            );
                        ...
                        else
                        {
                            ...
                            normal = se.transform.TransformPoint(
                                q * (v.point + v.normal) + startSample.location
                            );
                        }
                        se.SetToUpdate();
                    ...
                    else
                    {
                        ...
                        Vector3 movedNormal = Handles.Slider2D(
                            normal,
                            startSample.tangent,
                            Vector3.right,
                            Vector3.up,
                        ...
                        if (movedNormal != normal)
                        {
                            ...
                            v.normal =
                                (Vector2)(
                                    Quaternion.Inverse(q)
                                    * (
                                        se.transform.InverseTransformPoint(movedNormal)
                                        - startSample.location
                                    )
                                ) - v.point;
                            se.SetToUpdate();
                ...
                else
                {
                    ...
                    if (
                        GUI.Button(
                            new Rect(
                                p - new Vector2(QUAD_SIZE / 2, QUAD_SIZE / 2),
                ...
                Vector3 vAtSplineEnd = se.transform.TransformPoint(
                    q * next.point + startSample.location
                );
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.HelpBox(
                "Hold Alt and drag a vertex to create a new one.",
                MessageType.Info
            );
            ...
            if (vertices.isExpanded)
            {
                for (int i = 0; i < vertices.arraySize; i++)
                {
                    EditorGUILayout.PropertyField(
                        vertices.GetArrayElementAtIndex(i),
                        new GUIContent("Vertex " + i),
                        true
                    );
                }
            }
