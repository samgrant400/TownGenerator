...
namespace SplineMesh
{
    ...
    public class SplineEditor : Editor
    {
        ...
        {
            get { return (Spline)serializedObject.targetObject; }
        }
        ...
        void OnSceneGUI()
        {
            ...
            {
                Handles.DrawBezier(
                    spline.transform.TransformPoint(curve.n1.Position),
            ...
            switch (selectionType)
            {
                case SelectionType.Node:
                    ...
                    Vector3 newPosition = spline.transform.InverseTransformPoint(
                        Handles.PositionHandle(
                            spline.transform.TransformPoint(selection.Position),
                ...
                case SelectionType.Direction:
                    var result = Handles.PositionHandle(
                        spline.transform.TransformPoint(selection.Direction),
                        Quaternion.identity
                    );
                ...
                case SelectionType.InverseDirection:
                    result = Handles.PositionHandle(
                        2 * spline.transform.TransformPoint(selection.Position)
                            - spline.transform.TransformPoint(selection.Direction),
                        Quaternion.identity
                    );
                    selection.Direction =
                        2 * selection.Position - spline.transform.InverseTransformPoint(result);
                    break;
                case SelectionType.Up:
                    result = Handles.PositionHandle(
                        spline.transform.TransformPoint(selection.Position + selection.Up),
                        Quaternion.LookRotation(selection.Direction - selection.Position)
                    );
                    selection.Up =
                        (
                            spline.transform.InverseTransformPoint(result) - selection.Position
                        ).normalized;
                    break;
            }
        ...
        bool Button(Vector2 position, GUIStyle style)
        {
            return GUI.Button(
                new Rect(
                    position - new Vector2(QUAD_SIZE / 2, QUAD_SIZE / 2),
                    new Vector2(QUAD_SIZE, QUAD_SIZE)
                ),
                GUIContent.none,
                style
            );
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Add node after selected"))
            {
                ...
                SplineNode newNode = new SplineNode(
                    selection.Direction,
                    selection.Direction + selection.Direction - selection.Position
                );
