    ...
    public class CubicBezierCurve
    {
        ...
        public CurveSample GetSample(float time)
        {
            ...
            float t =
                next == previous
                    ? 0
                    : (time - previous.timeInCurve) / (next.timeInCurve - previous.timeInCurve);
        ...
        public CurveSample GetSampleAtDistance(float d)
        {
            if (d < 0 || d > Length)
                throw new ArgumentException(
                    "Distance must be positive and less than curve length. Length = "
                        + Length
                        + ", given distance was "
                        + d
                );
            ...
            float t =
                next == previous
                    ? 0
                    : (d - previous.distanceInCurve)
                      / (next.distanceInCurve - previous.distanceInCurve);
        ...
        public CurveSample GetProjectionSample(Vector3 pointToProject)
        {
            ...
            {
                var toPreviousSample =
                    (pointToProject - samples[closestIndex - 1].location).sqrMagnitude;
                var toNextSample =
                    (pointToProject - samples[closestIndex + 1].location).sqrMagnitude;
            ...
            var onCurve =
                Vector3.Project(
                    pointToProject - previous.location,
                    next.location - previous.location
                ) + previous.location;
            var rate =
                (onCurve - previous.location).sqrMagnitude
                / (next.location - previous.location).sqrMagnitude;
