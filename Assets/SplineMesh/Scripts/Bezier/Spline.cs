...
namespace SplineMesh
{
    ...
    public class Spline : MonoBehaviour
    {
        ...
        private void Reset()
        {
            ...
            RaiseNodeListChanged(
                new ListChangedEventArgs<SplineNode>() { type = ListChangeType.clear }
            );
        ...
        private int GetNodeIndexForTime(float t)
        {
            if (t < 0 || t > nodes.Count - 1)
            {
                throw new ArgumentException(
                    string.Format(
                        "Time must be between 0 and last node index ({0}). Given time was {1}.",
                        nodes.Count - 1,
                        t
                    )
                );
	...
	/// Refreshes the spline's internal list of curves.
        ...
        public CurveSample GetSampleAtDistance(float d)
        {
            if (d < 0 || d > Length)
                throw new ArgumentException(
                    string.Format(
                        "Distance must be between 0 and spline length ({0}). Given distance was {1}.",
                        Length,
                        d
                    )
                );
        ...
        public void AddNode(SplineNode node)
        {
            ...
            RaiseNodeListChanged(
                new ListChangedEventArgs<SplineNode>()
                {
        ...
        public void RemoveNode(SplineNode node)
        {
            ...
            {
                throw new Exception(
                    "Can't remove the node because a spline needs at least 2 nodes."
                );
            }

            CubicBezierCurve toRemove =
                index == nodes.Count - 1 ? curves[index - 1] : curves[index];
